...
const DEFAULT_ENABLED_SCOPES: [&str; 2] = ["rust.ir_lint", "markdown.chunk"];
...
fn run_lint_rust(args: LintRustArgs, _dag_trace_out: Option<&Path>) -> Result<(), String> {
    ...
    if !scope_is_enabled(&scope_enablement.enabled, "rust.ir_lint") {
        ...
        return Err(format!(
            "scope rust.ir_lint is disabled by {} (enable it under [scopes].enabled)",
            source
        ));
    }
...
fn run_ingest_dir(
    ...
    let scope_enablement = resolve_scope_enablement(&args.path)?;
    let obsidian_scope_enabled = scope_is_enabled(&scope_enablement.enabled, "obsidian.links")
        || scope_is_enabled(&scope_enablement.enabled, "vault.ir_lint");
...
fn resolve_scope_enablement(root_hint: &Path) -> Result<ScopeEnablement, String> {
...
fn scope_is_enabled(enabled_scopes: &[String], scope: &str) -> bool {
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("admit-scope-config-{}-{}", label, nanos))
    }
    ...
    fn resolve_scope_enablement_uses_defaults_when_missing() {
        ...
        assert!(scope_is_enabled(&resolved.enabled, "rust.ir_lint"));
        assert!(scope_is_enabled(&resolved.enabled, "markdown.chunk"));
    }
