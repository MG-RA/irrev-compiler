...
fn temp_ledger_path() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("admit-ledger-check-{}.jsonl", nanos))
}
...
fn temp_artifacts_dir() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("admit-artifacts-check-{}", nanos))
}

#[test]
fn check_appends_admissibility_checked_event() {
