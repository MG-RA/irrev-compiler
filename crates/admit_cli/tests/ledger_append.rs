...
use admit_cli::{append_event, declare_cost, DeclareCostError, DeclareCostInput};
...
fn temp_ledger_path() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("admit-ledger-{}.jsonl", nanos))
}
...
fn temp_artifacts_dir() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .expect("time")
        .as_nanos();
    std::env::temp_dir().join(format!("admit-artifacts-ledger-{}", nanos))
}
...
fn append_rejects_duplicate_event_id() {
    ...
    append_event(&ledger_path, &event).expect("first append");
    let err = append_event(&ledger_path, &event).expect_err("duplicate append");

    match err {
        DeclareCostError::DuplicateEventId(_) => {}
